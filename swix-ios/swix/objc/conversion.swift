...
public func matrixToPointer(_ x: vector)->UnsafeMutablePointer<Double>{
    // sustains since objc rewrites raw memory!
    return UnsafeMutablePointer<Double>(mutating: x.grid)
}
public func matrixToPointer(_ x: [Int])->UnsafeMutablePointer<Int>{
    return UnsafeMutablePointer<Int>(mutating: x)
}

/// use !x to get the address. I tried &x but that doesn't work in beta3.
prefix public func ! (x: vector) -> UnsafeMutablePointer<Double> {
    return matrixToPointer(x)
}
prefix public func ! (x: matrix) -> UnsafeMutablePointer<Double> {
